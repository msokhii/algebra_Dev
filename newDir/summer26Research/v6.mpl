...
MRFI := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    ...
    while true do
        for j from T_old to 2*T - 1 do
            sigma_ := [op(sigma_),
                       [seq(Primes[i]^j mod p, i=1..nops(Primes))]]:
            if lin_sys then
                mqrfr_results := Deterministic_NDSA(B, sigma_[j], direction,
                                                    num_vars, p,
                                                    num_points_mqrfr,
                                                    max_num_points_mqrfr,
                                                    num_eqn):
            else
                mqrfr_results, lin_sys := NDSA(B, sigma_[j], direction,
                                                num_vars, p,
            ...
            for k from 1 to num_eqn do
                if not numerator_done[k] then
                    num_eval[k] := [op(num_eval[k]),
                                    eval(Numerators[k], x=sigma_[j][1]) mod p]:
                end if;
                if not denominator_done[k] then
                    den_eval[k] := [op(den_eval[k]),
                                    eval(Denominiators[k], x=sigma_[j][1]) mod p]:
                end if;
            end do;
        end do;
        ...
        if T > 2^14 then
            print("MRFI: hit safety break at T=", T);
            break;
        end if;
    end do;
...
get_data := proc(test_case)
    ...
    if nargs = 1 then
        ...
        if test_case = "rand" then
            ...
            #   T_{ij} = y_{|i-j|+1},  RHS  b_i = i.
            n := args[2]:
            Vars   := [seq(x||i, i=1..n)]:
            params := [seq(y||i, i=1..n)]:
            Sys := { seq( add( y||(abs(ii-jj)+1) * x||jj, jj=1..n ) - ii,
                          ii = 1..n ) }:
    ...
    if status = "OK" then
        ...
        if do_verify then
            ...
            try
                ...
                if all_match then
                    printf("  PASS  n=%2d   MRFI=%.2fs  ref=%.2fs  BB-calls=%d\n",
                           n_test, t_mrfi, t_ref, counter):
                    summary := [op(summary),
                                [n_test, "PASS", t_mrfi, t_ref, counter]]:
                else
                    printf("  FAIL  n=%2d (mismatch)  MRFI=%.2fs  ref=%.2fs\n",
                           n_test, t_mrfi, t_ref):
                    summary := [op(summary),
                                [n_test, "FAIL-mismatch", t_mrfi, t_ref, counter]]:
                end if;
            catch:
                ...
                summary := [op(summary),
                            [n_test, "NO-VERIFY", t_mrfi, infinity, counter]]:
            end try;
        else
            printf("  RECOVERED (unverified) n=%2d  MRFI=%.2fs  BB-calls=%d\n",
                   n_test, t_mrfi, counter):
            for i from 1 to num_eqn do
                printf("    x%d = %a\n", i, Final_rat_poly[i]):
            end do;
            summary := [op(summary),
                        [n_test, "UNVERIFIED", t_mrfi, infinity, counter]]:
        end if;
    else
        printf("  FAIL  n=%2d   %s\n", n_test, status):
        summary := [op(summary),
                    [n_test, status, t_mrfi, infinity, counter]]:
    end if;
end do;
