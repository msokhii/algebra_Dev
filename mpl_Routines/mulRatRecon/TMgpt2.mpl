...
Benchmark_BB_Call := proc(B, point_::list, p::prime)
    local reps, t0, elapsed, i, dummy, counter_save, bb_time_save;
    global counter, bb_time_total;
    ...
    # B(point_,p) increments counter and bb_time_total.  These benchmark
    # calls must not be counted as real MRFI/NDSA black-box calls, so save
    # and restore the globals.
    counter_save := counter:
    bb_time_save := bb_time_total:

    t0 := time():
    for i from 1 to reps do
        dummy := B(point_, p):
    end do:
    elapsed := evalf(time() - t0):

    counter := counter_save:
    bb_time_total := bb_time_save:

    return evalf(elapsed/reps), elapsed;
end proc:
...
NDSA := proc(B, sigma_, beta_, num_var, p, num_points, num_eqn)
    local correct_degree, T, alpha, m, Psi_alpha, Y, u, dq, i, r,
          ...
          bb_avg_this_T, bb_elapsed_1000_this_T,
          bb_calls_this_T, bb_time_this_T,
          latest_T, latest_bb_avg, latest_bb_elapsed_1000,
          latest_bb_calls, latest_bb_time;
    global ndsa_stats;
    ...
    while not correct_degree do
        ...
        # Benchmark one BB call 1000 times for this T.
        # Keep overwriting these values; when this T succeeds, these are the values kept.
        bb_avg_this_T, bb_elapsed_1000_this_T := Benchmark_BB_Call(B, Psi_alpha[1], p):

        # Actual NDSA black-box calls for this T.
        Y := [seq(B(Psi_alpha[i], p), i=1..T)]:

        bb_calls_this_T := T:
        bb_time_this_T  := evalf(bb_avg_this_T * bb_calls_this_T):

        latest_T := T:
        latest_bb_avg := bb_avg_this_T:
        latest_bb_elapsed_1000 := bb_elapsed_1000_this_T:
        latest_bb_calls := bb_calls_this_T:
        latest_bb_time := bb_time_this_T:
        ...
        if dq > 1 then
            ...
            ndsa_stats["T_used"] := latest_T:
            ndsa_stats["bb_avg_per_call"] := latest_bb_avg:
            ndsa_stats["bb_time_1000_calls"] := latest_bb_elapsed_1000:
            ndsa_stats["bb_calls_correct_T"] := latest_bb_calls:
            ndsa_stats["bb_time_correct_T"] := latest_bb_time:
...
MRFI := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    local i, j, k, s, Primes, direction, sigma_, num_eval, den_eval, u, mon,
          ...
          ndsa_T_used, ndsa_bb_avg, ndsa_bb_time_1000,
          ndsa_bb_calls, ndsa_bb_time,
          round_counter_start, mrfi_bb_avg_this_Tcur,
          mrfi_bb_time_1000_this_Tcur, mrfi_bb_calls_this_Tcur,
          mrfi_bb_time_this_Tcur, latest_mrfi_Tcur,
          latest_mrfi_bb_avg, latest_mrfi_bb_time_1000,
          latest_mrfi_bb_calls, latest_mrfi_bb_time,
          bb_calls_simple_total, bb_time_simple_total;
    global mrfi_stats, ndsa_stats, counter;
    ...
    latest_mrfi_Tcur := 0:
    latest_mrfi_bb_avg := 0.0:
    latest_mrfi_bb_time_1000 := 0.0:
    latest_mrfi_bb_calls := 0:
    latest_mrfi_bb_time := 0.0:

    mqrfr_results, lin_sys := NDSA(B, init_sigma, direction,
                                    num_vars, p, Tcur, num_eqn):

    # Keep the timing only for the successful/final T used by NDSA.
    ndsa_T_used       := ndsa_stats["T_used"]:
    ndsa_bb_avg       := ndsa_stats["bb_avg_per_call"]:
    ndsa_bb_time_1000 := ndsa_stats["bb_time_1000_calls"]:
    ndsa_bb_calls     := ndsa_stats["bb_calls_correct_T"]:
    ndsa_bb_time      := ndsa_stats["bb_time_correct_T"]:
    ...
    while true do
        round_counter_start := counter:
        mrfi_bb_avg_this_Tcur := 0.0:
        mrfi_bb_time_1000_this_Tcur := 0.0:
        ...
        for j from jDone+1 to 2*Tcur do
            ...
            if lin_sys then
                ...
                # Benchmark one MRFI black-box call 1000 times once for this Tcur block.
                # Only the final successful Tcur block is kept.
                if j = jDone+1 then
                    mrfi_bb_avg_this_Tcur, mrfi_bb_time_1000_this_Tcur :=
                        Benchmark_BB_Call(B, Psi_alpha[1], p):
                end if:
        ...
        # Number of actual algorithm BB calls in this MRFI Tcur block.
        # Benchmark calls do not affect counter because Benchmark_BB_Call restores it.
        mrfi_bb_calls_this_Tcur := counter - round_counter_start:
        mrfi_bb_time_this_Tcur  := evalf(mrfi_bb_avg_this_Tcur * mrfi_bb_calls_this_Tcur):

        latest_mrfi_Tcur := Tcur:
        latest_mrfi_bb_avg := mrfi_bb_avg_this_Tcur:
        latest_mrfi_bb_time_1000 := mrfi_bb_time_1000_this_Tcur:
        latest_mrfi_bb_calls := mrfi_bb_calls_this_Tcur:
        latest_mrfi_bb_time := mrfi_bb_time_this_Tcur:
    ...
    bb_calls_simple_total := ndsa_bb_calls + latest_mrfi_bb_calls:
    bb_time_simple_total  := evalf(ndsa_bb_time + latest_mrfi_bb_time):
    ...
    # Simple BB timing model requested:
    #   1. In NDSA, benchmark one BB call 1000 times for each T.
    #      Keep the benchmark only for the T that succeeds.
    #   2. In MRFI, benchmark one BB call 1000 times for each Tcur block.
    #      Keep the benchmark only for the Tcur block that succeeds.
    #   3. Add the two estimated times at the end.
    mrfi_stats["NDSA_T_used"]          := ndsa_T_used:
    mrfi_stats["NDSA_BB_avg"]          := ndsa_bb_avg:
    mrfi_stats["NDSA_BB_time_1000"]    := ndsa_bb_time_1000:
    mrfi_stats["NDSA_BB_calls"]        := ndsa_bb_calls:
    mrfi_stats["NDSA_BB_est_time"]     := ndsa_bb_time:

    mrfi_stats["MRFI_Tcur_used"]       := latest_mrfi_Tcur:
    mrfi_stats["MRFI_BB_avg"]          := latest_mrfi_bb_avg:
    mrfi_stats["MRFI_BB_time_1000"]    := latest_mrfi_bb_time_1000:
    mrfi_stats["MRFI_BB_calls"]        := latest_mrfi_bb_calls:
    mrfi_stats["MRFI_BB_est_time"]     := latest_mrfi_bb_time:

    mrfi_stats["BB_calls_simple_total"] := bb_calls_simple_total:
    mrfi_stats["BB_time_simple_total"]  := bb_time_simple_total:
...
for n_test from n_min to n_max do
    ...
    # No end benchmark anymore.  Use the internal 1000-call benchmarks
    # kept for the accepted NDSA T and accepted MRFI Tcur.
    if status = "OK" then
        mrfi_calls      := mrfi_stats["BB_calls_simple_total"]:
        bb_total_est    := mrfi_stats["BB_time_simple_total"]:
        bb_actual_total := bb_total_est:
        if mrfi_calls > 0 then
            bb_per_call := evalf(bb_total_est / mrfi_calls):
        else
            bb_per_call := 0.0:
        end if:
    else
    ...
    if status = "OK" then
        ...
        stats_ndsa_T    := mrfi_stats["NDSA_T_used"]:
        stats_mrfi_Tcur := mrfi_stats["MRFI_Tcur_used"]:
        stats_ndsa_time := mrfi_stats["NDSA_BB_est_time"]:
        stats_mrfi_time := mrfi_stats["MRFI_BB_est_time"]:
    else
        stats_terms_num := []: stats_terms_den := []:
        stats_deg_num   := []: stats_deg_den   := []:
        stats_common    := false: stats_mMax := 0:
        stats_ndsa_T := 0: stats_mrfi_Tcur := 0:
        stats_ndsa_time := 0.0: stats_mrfi_time := 0.0:
    end if;
    ...
    if status = "OK" then
        ...
        if do_verify then
            try
                ...
                if all_match then
                    printf("  PASS  n=%2d   "
                           "BB-calls=%d  bb-total=%.3f s  NDSA-T=%d  MRFI-Tcur=%d\n",
                           n_test, mrfi_calls, bb_total_est,
                           stats_ndsa_T, stats_mrfi_Tcur):
                    summary := [op(summary),
...
printf("%4s  %-15s  %10s  %12s  %12s  %12s  %14s\n",
       "n", "status", "BB-calls", "bb/call(s)", "bb-est(s)", "bb-simple(s)",
       "FFGE-total(s)"):
...
for entry in summary do
    ...
    fprintf(fd, "  Total BB time (est)      : %.9f  (bb/call * BB-calls)\n",
            entry[5]):
    fprintf(fd, "  Total BB time (simple)   : %.9f  (accepted NDSA T + accepted MRFI Tcur)\n",
            entry[20]):
