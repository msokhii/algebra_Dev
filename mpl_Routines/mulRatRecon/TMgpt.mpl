...
NDSA := proc(B, sigma_, beta_, num_var, p, num_points, num_eqn)
    local correct_degree, T, alpha, m, Psi_alpha, Y, u, dq, i, r,
          ...
          c0, bt0, blk_calls, blk_time, blk_T,
          last_calls, last_time, last_T;

    global counter, bb_time_total, ndsa_stats;

    print("In NDSA");

    if not assigned(counter) then counter := 0 end if:
    if not assigned(bb_time_total) then bb_time_total := 0.0 end if:
    ...
    while not correct_degree do
        ...
        # Time only the BB calls made for this candidate T.
        blk_T := T:
        c0 := counter:
        bt0 := bb_time_total:
        ...
        blk_calls := counter - c0:
        blk_time  := evalf(bb_time_total - bt0):

        last_calls := blk_calls:
        last_time  := blk_time:
        last_T     := blk_T:
        ...
        if dq > 1 then
            ...
            ndsa_stats := table():
            ndsa_stats["success"] := true:
            ndsa_stats["T_used"] := last_T:
            ndsa_stats["bb_calls_latest_T"] := last_calls:
            ndsa_stats["bb_time_latest_T"] := last_time:
            ndsa_stats["lin_sys"] := lin_sys:
            ndsa_stats["iterations"] := count:
...
MRFI := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    local i, j, k, s, Primes, direction, sigma_, num_eval, den_eval, u, mon,
          ...
          mrfi_c0, mrfi_bt0,
          ndsa_c0, ndsa_bt0,
          initial_ndsa_total_calls, initial_ndsa_total_time,
          initial_ndsa_latest_calls, initial_ndsa_latest_time, initial_ndsa_T,
          round_c0, round_bt0, round_calls, round_time,
          sampling_cum_calls, sampling_cum_time,
          latest_round_calls, latest_round_time, latest_round_Tcur;

    global mrfi_stats, ndsa_stats, counter, bb_time_total;

    if not assigned(counter) then counter := 0 end if:
    if not assigned(bb_time_total) then bb_time_total := 0.0 end if:

    mrfi_c0  := counter:
    mrfi_bt0 := bb_time_total:

    sampling_cum_calls := 0:
    sampling_cum_time  := 0.0:
    latest_round_calls := 0:
    latest_round_time  := 0.0:
    latest_round_Tcur  := 0:
    ...
    ndsa_c0  := counter:
    ndsa_bt0 := bb_time_total:

    mqrfr_results, lin_sys := NDSA(B, init_sigma, direction,
                                    num_vars, p, Tcur, num_eqn):

    initial_ndsa_total_calls := counter - ndsa_c0:
    initial_ndsa_total_time  := evalf(bb_time_total - ndsa_bt0):

    initial_ndsa_latest_calls := ndsa_stats["bb_calls_latest_T"]:
    initial_ndsa_latest_time  := ndsa_stats["bb_time_latest_T"]:
    initial_ndsa_T            := ndsa_stats["T_used"]:
    ...
    while true do
        ...
        round_calls := counter - round_c0:
        round_time  := evalf(bb_time_total - round_bt0):

        sampling_cum_calls := sampling_cum_calls + round_calls:
        sampling_cum_time  := sampling_cum_time  + round_time:

        latest_round_calls := round_calls:
        latest_round_time  := round_time:
        latest_round_Tcur  := Tcur:
    ...
    # Initial NDSA timing.
    # latest_T means the T that actually gave the accepted MQRFR degrees.
    mrfi_stats["initial_ndsa_T_used"]         := initial_ndsa_T:
    mrfi_stats["initial_ndsa_calls_latest_T"] := initial_ndsa_latest_calls:
    mrfi_stats["initial_ndsa_time_latest_T"]  := initial_ndsa_latest_time:
    mrfi_stats["initial_ndsa_calls_total"]    := initial_ndsa_total_calls:
    mrfi_stats["initial_ndsa_time_total"]     := initial_ndsa_total_time:

    # MRFI sampling-loop timing.
    # latest_Tcur means the Tcur value at which BMEA finished.
    mrfi_stats["Tcur_used"]                   := latest_round_Tcur:
    mrfi_stats["sampling_calls_latest_Tcur"]  := latest_round_calls:
    mrfi_stats["sampling_time_latest_Tcur"]   := latest_round_time:
    mrfi_stats["sampling_calls_cumulative"]   := sampling_cum_calls:
    mrfi_stats["sampling_time_cumulative"]    := sampling_cum_time:

    # Main number to report if you want only the right/latest T values.
    mrfi_stats["bb_calls_latest_used"] :=
        initial_ndsa_latest_calls + latest_round_calls:

    mrfi_stats["bb_time_latest_used"] :=
        evalf(initial_ndsa_latest_time + latest_round_time):

    # Full actual BB time/calls during this MRFI run, including failed earlier T values.
    mrfi_stats["bb_calls_actual_total"] := counter - mrfi_c0:
    mrfi_stats["bb_time_actual_total"]  := evalf(bb_time_total - mrfi_bt0):
...
for n_test from n_min to n_max do
    ...
    # Do NOT benchmark B after MRFI.
    # These times/calls are measured inside NDSA and MRFI using the real BB calls.
    if status = "OK" then
        mrfi_calls      := mrfi_stats["bb_calls_latest_used"]:
        bb_latest_total := mrfi_stats["bb_time_latest_used"]:

        ndsa_latest_calls    := mrfi_stats["initial_ndsa_calls_latest_T"]:
        ndsa_latest_time     := mrfi_stats["initial_ndsa_time_latest_T"]:
        ndsa_T_used          := mrfi_stats["initial_ndsa_T_used"]:

        sampling_latest_calls := mrfi_stats["sampling_calls_latest_Tcur"]:
        sampling_latest_time  := mrfi_stats["sampling_time_latest_Tcur"]:
        sampling_Tcur_used    := mrfi_stats["Tcur_used"]:

        actual_calls_total := mrfi_stats["bb_calls_actual_total"]:
        actual_time_total  := mrfi_stats["bb_time_actual_total"]:

        if mrfi_calls > 0 then
            bb_per_call := evalf(bb_latest_total / mrfi_calls):
        else
            bb_per_call := 0.0:
        end if:
        ...
        # If MRFI failed before publishing mrfi_stats, report whatever was accumulated.
        mrfi_calls      := counter:
        bb_latest_total := bb_time_total:
        if mrfi_calls > 0 then
            bb_per_call := evalf(bb_latest_total / mrfi_calls):
        else
            bb_per_call := 0.0:
        ...
        ndsa_latest_calls     := 0:
        ndsa_latest_time      := 0.0:
        ndsa_T_used           := 0:
        sampling_latest_calls := 0:
        sampling_latest_time  := 0.0:
        sampling_Tcur_used    := 0:
        actual_calls_total    := counter:
        actual_time_total     := bb_time_total:
    ...
    if status = "OK" then
        ...
        if do_verify then
            try
                ...
                if all_match then
                    printf("  PASS  n=%2d  BB-used=%d  BB-time=%.6f s  NDSA(T=%d,calls=%d,time=%.6f)  MRFI(Tcur=%d,calls=%d,time=%.6f)\n",
                           n_test, mrfi_calls, bb_latest_total,
                           ndsa_T_used, ndsa_latest_calls, ndsa_latest_time,
                           sampling_Tcur_used, sampling_latest_calls, sampling_latest_time):

                    summary := [op(summary),
                                [n_test, "PASS", mrfi_calls,
                                 bb_per_call, bb_latest_total,
                                 stats_terms_num, stats_terms_den,
                                 stats_deg_num,   stats_deg_den,
                                 ...
                                 bb_latest_total,
                                 ndsa_latest_calls, ndsa_latest_time, ndsa_T_used,
                                 sampling_latest_calls, sampling_latest_time, sampling_Tcur_used,
                                 actual_calls_total, actual_time_total]]:
                else
            ...
            printf("  RECOVERED (unverified) n=%2d  BB-used=%d  BB-time=%.6f s\n",
                   n_test, mrfi_calls, bb_latest_total):
...
print("  SUMMARY"):
print("=================================================================="):
printf("%4s  %-15s  %10s  %12s  %12s  %8s  %12s  %8s  %12s  %14s\n",
       "n", "status", "BB-used", "bb/call(s)", "BB-used(s)",
       "NDSA-T", "NDSA(s)", "Tcur", "MRFI(s)", "FFGE-total(s)"):
for entry in summary do
    printf("%4d  %-15s  %10d  %12.3e  %12.6f  %8d  %12.6f  %8d  %12.6f  %14.6f\n",
           entry[1], entry[2], entry[3], entry[4], entry[5],
           entry[23], entry[22], entry[26], entry[25], entry[13]):
end do:

print(""):
print("  Full actual BB totals, including failed/exploratory T values"):
printf("%4s  %-15s  %14s  %14s\n",
       "n", "status", "actual calls", "actual time(s)"):
for entry in summary do
    printf("%4d  %-15s  %14d  %14.6f\n",
           entry[1], entry[2], entry[27], entry[28]):
end do:
...
for entry in summary do
    ...
    fprintf(fd, "  BB calls latest used     : %d\n",     entry[3]):
    fprintf(fd, "  BB time latest used      : %.9f\n",   entry[5]):
    fprintf(fd, "  Per call BB time         : %.9f\n",   entry[4]):
    fprintf(fd, "  Initial NDSA T used      : %d\n",     entry[23]):
    fprintf(fd, "  Initial NDSA BB calls    : %d\n",     entry[21]):
    fprintf(fd, "  Initial NDSA BB time     : %.9f\n",   entry[22]):
    fprintf(fd, "  MRFI sampling Tcur used  : %d\n",     entry[26]):
    fprintf(fd, "  MRFI sampling BB calls   : %d\n",     entry[24]):
    fprintf(fd, "  MRFI sampling BB time    : %.9f\n",   entry[25]):
    fprintf(fd, "  Actual BB calls total    : %d  (includes failed/exploratory T values)\n",
            entry[27]):
    fprintf(fd, "  Actual BB time total     : %.9f  (includes failed/exploratory T values)\n",
            entry[28]):
