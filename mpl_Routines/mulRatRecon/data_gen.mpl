...
get_data := proc(test_case, tSize)
    ...
    if nargs = 1 then
        ...
        if test_case = "bspline" then 
            ...
            gg := x-4;
            Vars := [x,y]:
            return Vars, ff, gg, nops(Vars), 1, Vars:

        else
            error "unknown one-argument test_case %1", test_case;
        end if:
        ...
        if test_case = "TOP" then
            Sys := MKTS(tSize):
        else
            error "two-argument call only supported for test_case = TOP";
        end if:
        ...
        Vars := [seq(parse(cat("x", i)), i=1..args[2])]:
        ...
        if test_case = "rand" then
            ...
            error "unknown multi-argument test_case %1", test_case;
        end if:
