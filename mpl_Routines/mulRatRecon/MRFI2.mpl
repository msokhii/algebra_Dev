...
MRFI2 := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    ...
    for j from 1 to 2*T-1 do
        ...
        for i from 1 to nops(deg_num) do
            ...
            alphaVal := [seq(tempG(), s=1..(tempDegNum+tempDegDenom+1))]:
            m := Expand(mul(x-alphaVal[s], s=1..nops(alphaVal))) mod p:

            Psi_alpha := get_point_on_affine_line(num_vars, alphaVal, direction, sigma_j, p,(tempDegNum+tempDegDenom+1)):
            Y := [seq(B(Psi_alpha[s], p)[i], s=1..nops(alphaVal))]:
...
MRFI2Fast := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    local i, j, k, s,
          Primes, direction, sigma_j,
          numerator_done, denominator_done, bmea_done,
          T, mqrfr_results, lin_sys,
          Numerators, Denominators, deg_num, deg_den,
          sampleCounts, m_i, mMax,
          lambda_num, lambda_den, terms_num, terms_den,
          R_num, R_den,
          final_num, final_den, coeff_num, coeff_den,
          common_den_flag,
          tempG, ratReconVal,
          num_eval, den_eval,
    ...
    # Initial degree discovery phase
    T := 4:
    mqrfr_results, T, lin_sys := NDSA(B, [seq(1, i=1..num_vars)], direction, num_vars, p, T, num_eqn):
    ...
    # Initial sample at x = 1
    num_eval := [seq([eval(Numerators[k], x=1) mod p], k=1..num_eqn)]:
    den_eval := [seq([eval(Denominators[k], x=1) mod p], k=1..num_eqn)]:

    tempG := rand(p):

    # Optional: prebuild modulus polynomials for prefixes of alphaVal
    modulusTable := table():
    ...
    for j from 1 to 2*T-1 do
        ...
        # If you want to be safer, you can enforce distinct alphaVal here.
        # For now this matches your existing random style.

        # Build prefix modulus polynomials:
        # modulusTable[m] = prod_{t=1..m} (x-alphaVal[t])
        modulusPoly := 1:
        ...
        # Recover each component using only its needed prefix length
        for i from 1 to num_eqn do
            m_i := sampleCounts[i]:

            Y := [seq(BBvals[s][i], s=1..m_i)]:

            interpVal := Interp(alphaVal[1..m_i], Y, x) mod p:
...
MRFI2Fast := proc(B, num_vars::integer, num_eqn::integer, vars::list, p::integer)
    ...
    lprint("MRFI2_global_adaptive ========================================"):
    lprint("MRFI2_global_adaptive Starting"):
    lprint("num_vars =", num_vars):
    lprint("num_eqn  =", num_eqn):
    lprint("=============================================================="):
